...
public class UpdateStudentProperty {
	...
	public static void main(String[] args) throws IOException, JDOMException {
		...
		try {
			...
			boolean result = updateStudentPropertyinXML(rollNumber, property, value, document);
			if (result) {
				...
				System.out.println("Student property has been updated successfully.");
			} else {
	...
	private static boolean updateStudentPropertyinXML(int rollNumber,String property, String value, Document document) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (studentRollNumber == rollNumber) {
				element = element.getChild(property);
				if (element != null) {
